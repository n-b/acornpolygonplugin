...
void psd_stream_get_object_contour(psd_uchar * lookup_table, psd_context * context)
{
	...
	// byte classID
	length = psd_stream_get_int(context);
	psd_assertnr(length == 0);
	key = psd_stream_get_int(context);
	// shape contour
	psd_assertnr(key == 'ShpC');
	...
	key = psd_stream_get_int(context);
	// curve
	psd_assertnr(key == 'Crv ');
	type = psd_stream_get_int(context);
	// list
	psd_assertnr(type == 'VlLs');
...
psd_blend_mode psd_stream_get_object_blend_mode(psd_context * context)
{
	...
	else
	{
		...
		keychar[length] = 0;
		if(strcmp((char const*)keychar, "linearBurn") == 0)
			blend_mode = psd_blend_mode_linear_burn;
		else if(strcmp((char const*)keychar, "linearDodge") == 0)
			blend_mode = psd_blend_mode_linear_dodge;
		else if(strcmp((char const*)keychar, "vividLight") == 0)
			blend_mode = psd_blend_mode_vivid_light;
		else if(strcmp((char const*)keychar, "linearLight") == 0)
			blend_mode = psd_blend_mode_linear_light;
		else if(strcmp((char const*)keychar, "pinLight") == 0)
			blend_mode = psd_blend_mode_pin_light;
		else if(strcmp((char const*)keychar, "hardMix") == 0)
		...
		switch(rootkey)
		{
			...
			case 'Clrs':	// color stop
				...
				for(i = 0; i < gradient_color->number_color_stops; i ++)
				{
					...
					key = psd_stream_get_int(context);
					// Location
					psd_assertnr(key == 'Lctn');
					
					// Type: OSType key
					type = psd_stream_get_int(context);
					// Integer
					psd_assertnr(type == 'long');
			...
			case 'Trns':	// tranparency stop
				...
				for(i = 0; i < gradient_color->number_transparency_stops; i ++)
				{
					...
					key = psd_stream_get_int(context);
					// opacity
					psd_assertnr(key == 'Opct');
					
					// Type: OSType key
					type = psd_stream_get_int(context);
					// Unit psd_float
					psd_assertnr(type == 'UntF');

					// '#Prc' = percent:
					key = psd_stream_get_int(context);
					psd_assertnr(key == '#Prc');
