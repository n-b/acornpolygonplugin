 	...
 	psd_int x2, psd_int y2)
{
	...
	*  which must be drawn
	*/
	boundary->vert_segs = (psd_int *)psd_malloc((x2 + 1) * sizeof(psd_int));
	...
	*/
	...
	boundary->empty_segs_n = (psd_int *)psd_malloc(boundary->max_empty_segs * sizeof(psd_int));
	memset(boundary->empty_segs_n, 0, boundary->max_empty_segs * 4);
	boundary->empty_segs_c = (psd_int *)psd_malloc(boundary->max_empty_segs * sizeof(psd_int));
	memset(boundary->empty_segs_c, 0, boundary->max_empty_segs * 4);
	boundary->empty_segs_l = (psd_int *)psd_malloc(boundary->max_empty_segs * sizeof(psd_int));
	...
	psd_int delta_wind, psd_double x, psd_double y)
{
	...
	if(swr->n_segs_max == seg_num)
	{
		...
		swr->svp = svp;
		swr->n_points_max = (psd_int *)psd_realloc(swr->n_points_max, swr->n_segs_max * sizeof(psd_int));
	}
...
static psd_art_svp_writer * psd_art_svp_writer_rewind_new(psd_art_wind_rule rule)
{
	...
	result->svp->n_segs = 0;
	result->n_points_max = (psd_int *)psd_malloc(result->n_segs_max * sizeof(psd_int));
